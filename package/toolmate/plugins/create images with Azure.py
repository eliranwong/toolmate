...
if config.online:
    ...
    from toolmate import print3, getCurrentDateTime, getCliOutput, azure_api_version
    ...
    @check_openai_errors
    def create_image_azure(function_args):
        ...
        client = AzureOpenAI(
            azure_endpoint=config.azureBaseUrl_dalle,
            api_version=azure_api_version,
            api_key=config.azureApi_key_dalle,
        )
        response = client.images.generate(
            model=config.azureOpenAI_model_dalle,
    ...
    config.addToolCall(signature=functionSignature, method=create_image_azure)