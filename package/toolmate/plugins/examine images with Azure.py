...
if config.online:
    ...
    from toolmate import print1, print2, is_valid_image_file, is_valid_image_url, is_valid_url, encode_image, getAzureClient
    from toolmate.utils.call_openai import check_openai_errors
    import os

    @check_openai_errors
    def examine_images_azure(function_args):
        from toolmate import config
        ...
        if content:
            ...
            response = getAzureClient().chat.completions.create(
                model="gpt-4o",
                messages=[
                    {
                    "role": "user",
