...
from toolmate import print1, print2, print3, check_llm_errors, getGroqClient, toParameterSchema, extractPythonCode, validParameters, refineToolTextOutput
...
def check_api_keys(func):
    def wrapper(*args, **kwargs):
        if isinstance(config.groqApi_key, str):
            ...
            for _ in range(len(config.groqApi_key)):
                try:
                    output = func(*args, **kwargs)
                    break
                except:
                    print(f"Failed API Key: {config.groqApi_key[0]}")
            return output
    return wrapper
...
```"""
        try:
            answer = CallGroq.getSingleChatResponse(content, temperature=0.0, keepSystemMessage=True)
            if not answer:
                answer = "high"
