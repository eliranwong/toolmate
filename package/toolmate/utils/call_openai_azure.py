...
from toolmate import print1, print2, print3, getDynamicTokens, validParameters, refineToolTextOutput, getAzureClient
...
def getSingleFunctionCallResponse(messages: list[dict], function_name: str, temperature=None, **kwargs):
    ...
    completion = getAzureClient().chat.completions.create(
...
class CallOpenAIAzure:
    ...
    def regularCall(messages: dict, **kwargs):
        ...
        return getAzureClient().chat.completions.create(
            model=config.chatGPTApiModel,
            messages=chatMessages,
            n=1,
