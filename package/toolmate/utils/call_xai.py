...
from toolmate import config, getRagPrompt, refineToolTextOutput
...
def autoCorrectPythonCode(code, trace):
    for i in range(config.max_consecutive_auto_correction):
        ...
        function_call_message, function_call_response = CallXAI.getSingleFunctionCallResponse(messages, "correct_python_code")
...
def getSingleChatResponse(userInput, messages=[], temperature: Optional[int]=None, max_tokens: Optional[int]=None, prefill: Optional[str]=None, stop: Optional[list]=None, keepSystemMessage: bool=False):
    ...
    try:
        completion = getXAIClient().chat.completions.create(
            model=config.xaiApi_tool_model,
            messages=chatMessages,
            n=1,
            temperature=temperature if temperature is not None else config.llmTemperature,
            max_tokens=max_tokens if max_tokens is not None else config.xaiApi_tool_model_max_tokens,
...
def getSingleFunctionCallResponse(messages: list[dict], function_name: str, temperature=None, **kwargs):
    ...
    completion = getXAIClient().chat.completions.create(
        ...
        n=1,
        temperature=temperature if temperature is not None else config.llmTemperature,
        max_tokens=config.xaiApi_tool_model_max_tokens,
    ...
    function_call_response = CallXAI.finetuneSingleFunctionCallResponse(func_arguments, function_name)
...
class CallXAI:
    ...
    def regularCall(messages: dict, **kwargs):
        ...
        return getXAIClient().chat.completions.create(
            model=config.xaiApi_tool_model,
            messages=chatMessages,
            n=1,
        ...
        if not config.selectedTool:
            ...
            try:
                if not tool_schema["parameters"]["properties"]:
                    ...
                    tool_parameters = CallXAI.getDictionaryOutput(messages=messages, schema=tool_schema)
    ...
    def extractToolParameters(schema: dict, ongoingMessages: list = [], **kwargs) -> dict:
        ...
        """
        parameters = CallXAI.getDictionaryOutput(messages=ongoingMessages, schema=schema, **kwargs)
        if config.developer:
            print2("```parameters")
