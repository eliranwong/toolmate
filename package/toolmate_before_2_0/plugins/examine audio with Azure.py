...
if config.online:
    ...
    from toolmate import showErrors, azure_api_version
    from toolmate import print2, print3
    from openai import AzureOpenAI
    import os

    # Function method
    def examine_audio_azure(function_args):
        from toolmate import config
        ...
        if audio_file and os.path.isfile(audio_file):
            ...
            try:
                client = AzureOpenAI(
                    azure_endpoint=config.azureBaseUrl_whisper,
                    api_version=azure_api_version,
                    api_key=config.azureApi_key_whisper,
                )
                with open(audio_file, "rb") as audio_file:
                    transcript = client.audio.transcriptions.create(
                        model=config.azureOpenAI_model_whisper, 
                        file=audio_file,
    ...
    config.addToolCall(signature=functionSignature, method=examine_audio_azure)