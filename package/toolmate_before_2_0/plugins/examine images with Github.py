...
if config.online:
    ...
    from toolmate import print1, print2, is_valid_image_file, is_valid_image_url, is_valid_url, encode_image, getGithubClient
    from toolmate.utils.call_openai import check_openai_errors
    import os

    @check_openai_errors
    def examine_images_github(function_args):
        from toolmate import config
        ...
        if content:
            ...
            response = getGithubClient().chat.completions.create(
                model="gpt-4o",
                messages=[
                    {
                    "role": "user",
    ...
    functionSignature = {
        ...
        "name": "examine_images_github",
        "description": "Describe or compare images with ChatGPT using Github API",
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
